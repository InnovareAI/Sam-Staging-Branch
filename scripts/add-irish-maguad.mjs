...
// Check existing workspace_accounts entry
const { data: wsAccount, error: wsError } = await supabase
...
if (wsError) {
  console.log('❌ Error finding workspace_accounts entry:', wsError.message);
  process.exit(1);
}

console.log('✅ Found workspace_accounts entry:');
console.log(`   Account: ${wsAccount.account_name}`);
console.log(`   Type: ${wsAccount.account_type}`);
console.log(`   Workspace: ${wsAccount.workspace_id}`);
console.log(`   User ID: ${wsAccount.user_id}`);
console.log('');

// Verify workspace_id matches InnovareAI
if (wsAccount.workspace_id !== INNOVARE_WORKSPACE_ID) {
  console.log(`⚠️  Workspace ID mismatch!`);
  console.log(`   Expected: ${INNOVARE_WORKSPACE_ID}`);
  console.log(`   Found: ${wsAccount.workspace_id}`);
  console.log('');
}

// Create user_unipile_accounts entry
...
console.log('✅ Successfully added Irish Maguad to user_unipile_accounts!');
console.log('');
console.log('Details:');
