-- Run this in Supabase SQL Editor to create enrichment_jobs table
...
DROP POLICY IF EXISTS "Users can create enrichment jobs" ON enrichment_jobs;
DROP POLICY IF EXISTS "Users can update their own enrichment jobs" ON enrichment_jobs;
DROP POLICY IF EXISTS "Service role can manage all enrichment jobs" ON enrichment_jobs;
...
DROP TRIGGER IF EXISTS enrichment_job_updated_at ON enrichment_jobs;
...
SELECT 'enrichment_jobs table created successfully!' AS status;
SELECT COUNT(*) AS existing_jobs FROM enrichment_jobs;
