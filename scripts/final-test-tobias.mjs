...
// IA7 workspace (Tobias)
const workspaceId = '85e80099-12f9-491a-a0a1-ad48d086a9f0';
const unipileAccountId = 'v8-RaHZzTD60o6EVwqcpvg';
...
const testedNames = [
  ...
  'Zachary Schmidt', 'Zachary J. Schmidt', 'Ben Nevile'
];

console.log('\n🔍 Finding untouched prospect in Tobias IA7 workspace...\n');

// Find campaigns in IA7 workspace with CR messages
...
for (const campaign of campaigns) {
  ...
  const untouched = prospects.find(p => {
    ...
    return !testedNames.includes(fullName) &&
           ...
           p.linkedin_url && // Must have LinkedIn URL
           p.first_name && p.last_name; // Must have names
  });
...
if (!selectedProspect || !selectedCampaign) {
  console.log('❌ No untouched prospects found in IA7 workspace with CR messages\n');
  process.exit(1);
}
...
console.log(`   Company: ${selectedProspect.company_name || 'N/A'}`);
console.log(`   LinkedIn: ${selectedProspect.linkedin_url}`);
console.log(`   Account: Tobias Linz (IA7)`);
...
const crMessage = selectedCampaign.message_templates.connection_request
  ...
  .replace(/{company_name}/g, selectedProspect.company_name || 'your company')
...
if (updated.status === 'connection_requested' && updated.contacted_at) {
  ...
  console.log(`\n   Check Tobias Linz LinkedIn for connection request to:`);
  console.log(`   ${selectedProspect.first_name} ${selectedProspect.last_name} (${selectedProspect.company_name || 'N/A'})`);
  console.log(`   Message: "${crMessage.substring(0, 80)}..."\n`);
} else {
  console.log('\n❌ TEST FAILED!');
