#!/usr/bin/env node
/**
 * Trigger IA7 campaign via LOCAL Inngest endpoint for testing
 */
...
console.log('🚀 TRIGGERING IA7 CAMPAIGN VIA LOCAL INNGEST\n');

// Reset prospect to pending first
...
await supabase
  ...
  .update({ status: 'pending', contacted_at: null, sent_at: null })
  .eq('id', prospects[0].id);

console.log('✅ Prospect reset to pending\n');

try {
  // Send event to LOCAL endpoint
  const response = await fetch('http://localhost:3001/api/inngest', {
    method: 'POST',
    headers: {
  ...
  console.log('✅ Event sent to local Inngest endpoint!');
  ...
  const { data: prospect } = await supabase
    .from('campaign_prospects')
    .select('first_name, status, contacted_at')
    .eq('id', prospects[0].id)
    .single();

  console.log('📊 Prospect status after 10 seconds:');
  console.log(`   Name: ${prospect.first_name}`);
  console.log(`   Status: ${prospect.status}`);
  console.log(`   Contacted: ${prospect.contacted_at || 'null'}`);

  if (prospect.status !== 'pending') {
    console.log('\n✅✅✅ SUCCESS! Prospect status changed - function executed!\n');
  } else {
    console.log('\n❌ Function did not execute - prospect still pending\n');
  }
