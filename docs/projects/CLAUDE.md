...
- **Multi-Model Agentic Architecture** - See [Agentic System Architecture Guide](../ai/AGENTIC_SYSTEM_ARCHITECTURE_GUIDE.md)
- **OpenAI API** - GPT models for structured outputs and function calling
- **Anthropic Claude** - Constitutional AI for safety-sensitive reasoning and compliance
- **Perplexity API** - Real-time grounded retrieval and fact-checking
- **Google Gemini** - Multimodal analysis and long-context processing
- **Supabase Vector Embeddings** - Contextual retrieval with 67% accuracy improvement
- **MCP Integration Framework** - AI service orchestration with Bright Data, Apify, WebSearch
- **Custom Sales Intelligence** - MEDDIC/Challenger methodology with multi-agent research
...
**CRITICAL**: All AI agent development must follow the [Agentic System Architecture Guide](../ai/AGENTIC_SYSTEM_ARCHITECTURE_GUIDE.md)

#### **Multi-Model Development Principles**
1. **Task-Based Routing** - Route tasks to optimal AI models based on capability requirements
2. **Atomic Agent Design** - Keep agents narrowly scoped with clear responsibilities
3. **Compliance First** - Use Claude Sonnet for safety-sensitive tasks requiring constitutional AI
4. **Cost Optimization** - Use model hierarchy (Haiku → Sonnet → GPT-4 → Gemini) based on complexity
5. **Quality Assurance** - Implement cross-model validation and HITL loops for critical decisions

#### **Model Selection Matrix**
```typescript
// SAM AI Model Selection Guidelines
const modelSelection = {
  // Safety-sensitive tasks → Claude Sonnet
  'cross_company_invitations': 'claude-sonnet',
...
- **Enhanced MCP Registry**: Multi-model provider integration with intelligent routing
- **Compliance Engine**: Constitutional AI validation for all safety-sensitive operations
- **Contextual Retrieval**: 67% accuracy improvement through contextual embeddings + BM25
- **QA Validation**: Cross-model verification for output quality assurance
- **Audit Logging**: Complete traceability through enhanced error tracking system
