...
## 27. Google Chat Client Replies Channel (Dec 19)

### Problem

All LinkedIn campaign reply notifications were being sent to the InnovareAI team Google Chat channel, including replies from client accounts (Stan Bounev, Samantha Truman, Rony Chatterjee). Clients needed their own dedicated channel to see replies from their campaigns without mixing with IA internal campaigns.

### Solution

**Added dedicated client replies webhook and routing logic.**

### Environment Variable

**Added to Netlify:**
```bash
GOOGLE_CHAT_CLIENT_REPLIES_WEBHOOK_URL=https://chat.googleapis.com/v1/spaces/AAQA3hEJYnQ/messages
```

### Client Workspace IDs

| Client | Workspace ID |
|--------|--------------|
| Stan Bounev | `5b81ee67-4d41-4997-b5a4-e1432e060d12` |
| Samantha Truman | `dea5a7f2-673c-4429-972d-6ba5fca473fb` |
| Rony Chatterjee | `8a720935-db68-43e2-b16d-34383ec6c3e8` |

### Routing Hierarchy

**File: `lib/notifications/google-chat.ts`**

The system now routes LinkedIn reply notifications based on workspace ID:

1. **IA Workspaces** → `GOOGLE_CHAT_REPLIES_WEBHOOK_URL` (existing IA team channel)
2. **Client Replies (Stan/Sam/Rony)** → `GOOGLE_CHAT_CLIENT_REPLIES_WEBHOOK_URL` (new dedicated channel)
3. **Dedicated Clients** → `CLIENT_WORKSPACE_WEBHOOKS` (workspace-specific channels)
4. **Others** → `GOOGLE_CHAT_CLIENT_WEBHOOK_URL` (QC channel)

**Code Added (lines 45-51):**
...
function getClientRepliesWebhook(workspaceId: string): string | null {
  if (CLIENT_REPLIES_WORKSPACE_IDS.includes(workspaceId)) {
    return process.env.GOOGLE_CHAT_CLIENT_REPLIES_WEBHOOK_URL || null;
  }
  return null;
}
```

**Updated `sendReplyNotification()` function (lines 500-530):**

```typescript
// 1. Check for dedicated workspace webhook
const dedicatedWebhook = CLIENT_WORKSPACE_WEBHOOKS[workspace_id];
if (dedicatedWebhook) {
  webhookUrl = dedicatedWebhook;
}
// 2. Check for client replies webhook (Stan/Sam/Rony)
else if (CLIENT_REPLIES_WORKSPACE_IDS.includes(workspace_id)) {
  webhookUrl = process.env.GOOGLE_CHAT_CLIENT_REPLIES_WEBHOOK_URL || webhookUrl;
}
// 3. Use IA replies webhook for IA workspaces
else if (IA_WORKSPACE_IDS.includes(workspace_id)) {
  webhookUrl = process.env.GOOGLE_CHAT_REPLIES_WEBHOOK_URL || webhookUrl;
}
// 4. Fallback to QC webhook
else {
  webhookUrl = process.env.GOOGLE_CHAT_CLIENT_WEBHOOK_URL || webhookUrl;
}
```

### Features

All reply notifications include:
- Draft reply generated by AI
- Action buttons: "Send Reply", "Edit Reply", "View Conversation"
- @all mentions for visibility
- Campaign name, prospect name, and workspace info

### Files Modified

| File | Lines | Change |
|------|-------|--------|
| `lib/notifications/google-chat.ts` | 45-51 | Added `CLIENT_REPLIES_WORKSPACE_IDS` constant |
| `lib/notifications/google-chat.ts` | 500-530 | Updated routing logic in `sendReplyNotification()` |

### Testing

Sent test message successfully to verify webhook connectivity and routing logic.

### Result

Client accounts (Stan, Samantha, Rony) now receive their campaign reply notifications in a dedicated Google Chat channel, separate from InnovareAI's internal campaigns.
...
- [x] Add dedicated client replies Google Chat channel
