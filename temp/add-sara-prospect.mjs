...
// 1. Get a campaign to add her to (use the most recent active one)
...
if (!campaign) {
  console.log('❌ No active campaign found for Charissa');
  process.exit(1);
}

console.log(`\n✓ Using campaign: ${campaign.name} (${campaign.id})`);

// 2. Get Sara's profile from Unipile for accurate data
...
const { data: prospect, error } = await supabase
  ...
  .insert({
    ...
    status: 'replied',  // She already replied
    responded_at: '2025-12-16T00:11:34.603Z',  // When she replied
    connection_request_sent_at: '2025-12-02T15:16:55.919Z',  // When message was sent
...
console.log('\n📨 Creating reply agent draft for her message...');
...
console.log('\n   The Reply Agent will process her message on next cron run.');
console.log('='.repeat(70));
