        ...
        return (
          <Card key={c.label}>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              ...
              {!isLoading && c.trend && (
                <div className={`flex items-center text-xs font-medium ${c.trendUp ? 'text-green-500' : 'text-red-500'
                  }`}>
                  {c.trendUp ? '↗' : '↘'} {c.trend}
                </div>
              )}
...
const Analytics: React.FC<AnalyticsProps> = ({ workspaceId }) => {
  ...
  // Helper to get user initials from UUID
  const getUserDisplayName = (userId: string): string => {
    if (!userId) return '—';

    const member = workspaceMembers.find((m: any) => m.user_id === userId);

    // Get initials from full name (e.g., "Thorsten Linz" → "TL")
    if (member?.users?.full_name) {
      const nameParts = member.users.full_name.trim().split(/\s+/);
    ...
    // Get initials from email (e.g., "tl@company.com" → "TL")
    if (member?.users?.email) {
      const emailName = member.users.email.split('@')[0];
      return emailName.substring(0, 2).toUpperCase();
    }

    // Fallback: show first 2 chars of UUID (very last resort)
    return userId.substring(0, 2).toUpperCase();
  };

  // Use real campaign data if available, otherwise show demo data
  const baseCampaignRows = demoMode ? [
    { name: "Q4 Enterprise Outreach", owner: "SP", type: 'connector', prospects: 247, messages: 324, replies: 156, infoRequests: 67, meetings: 23, responseRate: 0.481, crSent: 247, crAccepted: 189, crRate: 77 },
    { name: "SaaS Founders Series", owner: "JS", type: 'messenger', prospects: 189, messages: 203, replies: 89, infoRequests: 42, meetings: 18, responseRate: 0.438, crSent: 0, crAccepted: 0, crRate: 0 },
    { name: "VP of Sales Target", owner: "EC", type: 'group', prospects: 156, messages: 178, replies: 71, infoRequests: 38, meetings: 15, responseRate: 0.399, crSent: 0, crAccepted: 0, crRate: 0 },
    { name: "Tech Startup Warmup", owner: "MB", type: 'connector', prospects: 134, messages: 121, replies: 62, infoRequests: 29, meetings: 9, responseRate: 0.512, crSent: 134, crAccepted: 98, crRate: 73 },
    { name: "FinTech Decision Makers", owner: "SP", type: 'email', prospects: 121, messages: 66, replies: 48, infoRequests: 20, meetings: 6, responseRate: 0.727, crSent: 0, crAccepted: 0, crRate: 0 },
  ] : campaignsData.map(c => ({
    name: c.campaign_name || 'Unnamed Campaign',
    owner: c.owner_initials || '—',
    type: c.campaign_type || 'multi_channel',
    prospects: c.prospects_count || 0,
              ...
              <div className="flex items-center gap-3">
                <div className="inline-flex rounded-lg border border-gray-600 overflow-hidden">
                  <button
                    ...
                    className={`px-4 py-2 text-sm font-medium transition-colors border-r border-gray-600 ${timeRange === '1d' ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                      }`}
                  >
                    1 day
                  </button>
                  <button
                    ...
                    className={`px-4 py-2 text-sm font-medium transition-colors border-r border-gray-600 ${timeRange === '7d' ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                      }`}
                  >
                    7 days
                  </button>
                  <button
                    ...
                    className={`px-4 py-2 text-sm font-medium transition-colors border-r border-gray-600 ${timeRange === '1m' ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                      }`}
                  >
                    1 month
                  </button>
                  <button
                    ...
                    className={`px-4 py-2 text-sm font-medium transition-colors border-r border-gray-600 ${timeRange === '3m' ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                      }`}
                  >
                    3 months
                  </button>
                  <button
                    ...
                    className={`px-4 py-2 text-sm font-medium transition-colors ${timeRange === 'custom' ? 'bg-purple-600 text-white' : 'bg-gray-700 text-gray-300 hover:bg-gray-600'
                      }`}
                  >
                    Custom
                  </button>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  {chartType === 'area' ? (
                    <AreaChart data={campaignSeries}>
                      ...
                      <XAxis dataKey="date" axisLine={false} tickLine={false} tick={{ fill: '#9CA3AF' }} tickFormatter={(value) => { try { return format(new Date(value), 'MMM d'); } catch { return value; } }} />
                ...
                <thead>
                  <tr className="border-b border-gray-700">
                    ...
                    {campaignType === 'connector' ? (
                      <>
                        <th className="text-right py-3 px-4 text-sm font-medium text-gray-400">CR Sent</th>
                        <th className="text-right py-3 px-4 text-sm font-medium text-gray-400">Connected</th>
                        ...
                        <th className="text-right py-3 px-4 text-sm font-medium text-gray-400">CR Rate</th>
                      </>
                    ) : (
                      <>
                ...
                <tbody>
                  {filteredCampaignRows.map((campaign, index) => (
                    ...
                    >
                      ...
                      {campaignType === 'connector' ? (
                        <>
                          <td className="py-3 px-4 text-right text-white">{campaign.crSent?.toLocaleString() || 0}</td>
                          <td className="py-3 px-4 text-right text-cyan-400">{campaign.crAccepted?.toLocaleString() || 0}</td>
