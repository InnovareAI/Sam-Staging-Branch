  ...
  try {
    ...
    for (const [workspaceId, wsMonitors] of monitorsByWorkspace) {
      ...
      for (const monitor of wsMonitors) {
        try {
          // Extract company slug from COMPANY:company_slug
          const companyHashtag = monitor.hashtags.find((h: string) => h.startsWith('COMPANY:'));
          if (!companyHashtag) continue;
          ...
          // Unipile uses /api/v1/companies/{identifier} for company lookup
