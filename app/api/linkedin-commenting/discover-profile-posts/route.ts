...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const monitor of profileMonitors) {
      ...
      const profileHashtags = monitor.hashtags.filter((h: string) => h.startsWith('PROFILE:'));

      for (const profileHashtag of profileHashtags) {
        if (totalDiscovered >= SCRAPING_LIMIT) break;
        ...
        try {
          ...
          console.error(`❌ Error processing profile ${profileHashtag}:`, error);
          continue;
        }
      } // End of profile loop
    } // End of monitor loop
