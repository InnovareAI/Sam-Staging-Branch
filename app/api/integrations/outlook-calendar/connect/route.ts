/**
 * Outlook Calendar Integration - Connect
 *
 * POST /api/integrations/outlook-calendar/connect
 *
 * Generates a Unipile hosted auth link for Outlook/Microsoft 365 Calendar
 *
  ...
  try {
    ...
    const response = await fetch(`https://${UNIPILE_DSN}/api/v1/hosted/accounts/link`, {
      ...
      body: JSON.stringify({
        ...
        expiresOn,
        name: `${workspace_id}-outlook-calendar`,
        notify_url: `${APP_URL}/api/webhooks/unipile/outlook-calendar`,
        success_redirect_url: `${APP_URL}/settings?connected=outlook-calendar`,
        failure_redirect_url: `${APP_URL}/settings?error=outlook-calendar`,
      }),
    });
    ...
    console.log(`📅 Generated Outlook Calendar connection link for workspace ${workspace_id}`);
