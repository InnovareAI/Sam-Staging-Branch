...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    console.log(`👥 Found ${allUsers.length} total SAM AI users`);

    // Step 4: Check existing associations
    console.log('🔗 Step 4: Checking existing associations...');
    const existingResult = await pool.query(
      'SELECT user_id, unipile_account_id, account_email FROM user_unipile_accounts'
    );
    const existingAssociations = existingResult.rows;
    ...
    const usersToProcess = target_users.length > 0 ?
      allUsers.filter((u: any) => target_users.includes(u.email)) :
      allUsers;

    for (const samUser of usersToProcess) {
      const samUserEmail = samUser.email?.toLowerCase();
      if (!samUserEmail) continue;

      console.log(`🔍 Processing user: ${samUserEmail}`);
      ...
      const matchingAccounts = linkedInAccounts.filter((account: any) => {
        ...
        return possibleEmails.includes(samUserEmail);
      });

      console.log(`📧 User ${samUserEmail}: Found ${matchingAccounts.length} matching accounts`);
      ...
      for (const matchingAccount of matchingAccounts) {
        ...
        try {
          ...
          `, [
            ...
            samUserEmail,
            matchingAccount.connection_params?.im?.publicIdentifier,
            matchingAccount.connection_params?.im?.publicIdentifier ?
          ...
          if (insertResult.rows.length > 0) {
            console.log(`✅ Successfully associated ${matchingAccount.id} with ${samUserEmail}`);
            rolloutResults.push({
              user_email: samUserEmail,
              user_id: samUser.id,
              unipile_account_id: matchingAccount.id,
          ...
          console.error(`❌ Failed to associate ${matchingAccount.id} with ${samUserEmail}:`, error);
          rolloutResults.push({
            user_email: samUserEmail,
            user_id: samUser.id,
            unipile_account_id: matchingAccount.id,
    ...
    return NextResponse.json({
      ...
      schema_check: schemaResult.rows || 'Schema check not available',
      recommendations: [
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const associationsResult = await pool.query(
      'SELECT user_id, unipile_account_id, account_email, account_name, created_at FROM user_unipile_accounts'
    );
    ...
    const statusReport = {
      ...
      users_without_linkedin: (allUsers?.length || 0) - userAssociationMap.size,
      rollout_percentage: allUsers?.length ?
        Math.round((userAssociationMap.size / allUsers.length) * 100) : 0
    };
