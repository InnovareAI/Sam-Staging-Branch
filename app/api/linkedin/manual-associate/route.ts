...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(`Manual association for user ${userEmail} (${userId}) with account ${unipile_account_id}`);
    ...
    for (const accountDetails of linkedinAccounts) {
      ...
      const { rows: existingRows } = await pool.query(
        `SELECT id, account_name
         ...
         WHERE workspace_id = $1 AND user_id = $2 AND unipile_account_id = $3`,
        [workspaceId, userId, accountDetails.id]
      );
      const existingAssociation = existingRows[0];
      ...
      try {
        ...
        console.error(`Failed to create association for ${accountDetails.id}:`, error);
        results.push({
          account_id: accountDetails.id,
          status: 'failed',
          error: error.message,
