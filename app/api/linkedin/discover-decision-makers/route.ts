...
import { checkSearchQuota } from '@/lib/linkedin';
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (authError || !session) return NextResponse.json({ error: 'Auth required' }, { status: 401 });
        ...
        if (!activeWorkspaceId) {
            const { data: userProfile } = await supabase.from('users').select('current_workspace_id').eq('id', session.user.id).single();
            activeWorkspaceId = userProfile?.current_workspace_id;
        }
        if (!activeWorkspaceId) return NextResponse.json({ error: 'No workspace found' }, { status: 400 });

        // 2. LinkedIn Account
        let linkedinAccountId = accountId;
        if (!linkedinAccountId) {
            const { data: accounts } = await supabase.from('workspace_accounts').select('unipile_account_id').eq('workspace_id', activeWorkspaceId).eq('account_type', 'linkedin').in('connection_status', VALID_CONNECTION_STATUSES).limit(1);
            if (!accounts || accounts.length === 0) return NextResponse.json({ error: 'No active LinkedIn account' }, { status: 400 });
        ...
        for (let i = 0; i < companyIds.length; i += BATCH_SIZE) {
            ...
            try {
                ...
                logger.error(`⚠️ [Discovery] Batch search failed`, err);
