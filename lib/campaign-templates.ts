export interface CampaignTemplate {
  ...
  name: string;
  description: string;
  type: 'linkedin_connection' | 'linkedin_dm' | 'group_message' | 'company_follow';
  steps: CampaignStep[];
  settings: {
    daily_limit: number;
    delay_between_steps_days: number;
...
export interface CampaignStep {
  step_number: number;
  type: 'connection_request' | 'follow_up_message' | 'direct_message' | 'group_message' | 'company_follow';
  delay_days: number;
  subject?: string;
...
export const getTemplatesByType = (type: CampaignTemplate['type']): CampaignTemplate[] => {
  return CAMPAIGN_TEMPLATES.filter(template => template.type === type);
};

// Helper function to validate template variables
export const validateTemplateVariables = (template: CampaignTemplate, prospectData: any): string[] => {
...
export const populateTemplate = (messageTemplate: string, prospectData: any): string => {
...
export const getRecommendedTemplate = (criteria: {
  relationship_status: 'not_connected' | 'connected' | 'group_member';
  campaign_goal: 'networking' | 'sales' | 'partnership' | 'thought_leadership';
  industry: string;
  urgency: 'low' | 'medium' | 'high';
}): CampaignTemplate => {
  
  // Logic to recommend best template based on criteria
  if (criteria.relationship_status === 'not_connected') {
    return CAMPAIGN_TEMPLATES[0]; // Connection request campaign
  } else if (criteria.relationship_status === 'connected') {
    return CAMPAIGN_TEMPLATES[1]; // Direct message campaign
  } else if (criteria.relationship_status === 'group_member') {
    return CAMPAIGN_TEMPLATES[2]; // Group message campaign
  }
  
  // Default to connection request campaign
  return CAMPAIGN_TEMPLATES[0];
};